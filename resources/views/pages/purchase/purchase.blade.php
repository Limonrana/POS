...
@section('title', 'Purchase | Inventory POS')
        ...
        <div class="card-body">
            <div class="row justify-content-between align-items-center">
                <div class="col-sm-auto mb-2 mb-sm-0">
                    <h6 class="mb-0">DashBoard > Purchase</h6>
                </div>
            ...
            <div class="card-header">
                <div class="row align-items-center justify-content-between">
                    <div class="col-6 col-sm-auto d-flex align-items-center pr-0">
                        <h5 class="fs-1 mb-0 text-nowrap py-2 py-xl-0">Purchase List</h5>
                    </div>
                    <div class="col-6 col-sm-auto ml-auto text-right pl-0">
                        ...
                        <div id="dashboard-actions">
                            <a href="{{ route('purchase.create') }}" class="btn btn-falcon-default btn-sm" type="button">
                ...
                <div class="dashboard-data-table">
                    <table class="table table-sm table-dashboard fs--1 data-table border-bottom" id='recordsTable' data-options='{"responsive":false,"pagingType":"simple","lengthChange":false,"searching":false,"pageLength":8,"columnDefs":[{"targets":[0,6],"orderable":false}],"language":{"info":"_START_ to _END_ Items of _TOTAL_ — <a href=https://prium.github.io/"#!\" class=\"font-weight-semi-bold\"> view all <span class=\"fas fa-angle-right\" data-fa-transform=\"down-1\"></span> </a>"},"buttons":["copy","excel"]}'>
                        ...
                        <tr>
                            ...
                            <th class="sort pr-1 align-middle">Vendor</th>
                        ...
                        @foreach($purchase as $key => $val)
                            <tr class="btn-reveal-trigger" id="value_{{ $val->id }}">
                                ...
                                <th class="align-middle"><a href="{{ route('purchase.edit', $val->id) }}" class="color-blue">{{ $val->order_number }}</a></th>
                                <td class="align-middle">{{ \Illuminate\Support\Str::limit($val->vendor->title_name, 6) }} - {{ \Illuminate\Support\Str::limit($val->vendor->name, 12) }}</td>
                                    ...
                                    <div class="dropdown text-sans-serif"><button class="btn btn-link text-600 btn-sm dropdown-toggle btn-reveal mr-3" type="button" id="dropdown0" data-toggle="dropdown" data-boundary="html" aria-haspopup="true" aria-expanded="false"><span class="fas fa-ellipsis-h fs--1"></span></button>
                                        <div class="dropdown-menu dropdown-menu-right border py-0" aria-labelledby="dropdown0">
                                            <div class="bg-white py-2">
                                                <a class="dropdown-item" href="{{ route('purchase.edit', $val->id) }}">Edit</a>
                                                <div class="dropdown-divider"></div>
                                                <a class="dropdown-item" href="{{ route('purchase.invoice', $val->id) }}">Invoice</a>
                                                <div class="dropdown-divider"></div>
                                                @if ($val->status == 0)
                                                <a class="dropdown-item" href="{{ route('purchase.fulfillment', $val->id) }}">Fulfill</a>
                                                @else
                                                <a class="dropdown-item" href="{{ route('purchase.fulfillment', $val->id) }}">Unfulfill</a>
                                                @endif
                                                <div class="dropdown-divider"></div>
                                                @if ($val->is_pay == 0)
                                                    <a class="dropdown-item" href="{{ route('purchase.pay', $val->id) }}">Pay</a>
                                                @elseif ($val->is_pay == 2)
                                                    <a class="dropdown-item" href="{{ route('purchase.pay', $val->id) }}">Pay Left</a>
                                                @else
                                                    <a class="dropdown-item" href="{{ route('purchase.unpay', $val->id) }}">Unpay</a>
                                                @endif
                                                <div class="dropdown-divider"></div>
            ...
            <div class="card-body overflow-hidden p-lg-7">
                <div class="row align-items-center justify-content-between">
                    ...
                    <div class="col-lg-6 pl-lg-4 my-5 text-center text-lg-left">
                        <h3>No More Purchase Here!</h3>
                        <p class="lead">Create a New Purchase and Start Something Beautiful.</p><a href="{{ route('purchase.create') }}" class="btn btn-falcon-primary">Add New</a>
                    </div>
                </div>
            </div>
        </div>
                ...
                if(data_arr.length > 0){
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            //Delete Request Send
                            axios.post('/all-purchase', { id: data_arr})
                                .then(response => {
